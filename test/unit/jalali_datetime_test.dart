...
void main() {
  ...
  group("comparison with shamsi date", () {
    test("leap years", () {
      print(Jalali(1403).isLeapYear());
      print(JalaliDateTime(1403).isLeapYear);
      for (int i = 1; i <= 2000; i++){
        JalaliDateTime jalaliDatetime = JalaliDateTime(i);
        Jalali jalali = Jalali(i);
        if (jalali.isLeapYear() != jalaliDatetime.isLeapYear) print("${jalali.isLeapYear() ? "correct" : "incorrect"} leap year: $i");
      }
    },);
  },);
